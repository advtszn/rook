...
export function AddConnectionModal({ onSave, onCancel }: Props) {
	...
	const handleSave = useCallback(() => {
		...
		if (Number.isNaN(database) || database < 0 || database > 15) {
			setError("Database must be 0-15");
			return;
		}
	...
	usePaste((event) => {
		const text = new TextDecoder()
			.decode(event.bytes)
			// biome-ignore lint/suspicious/noControlCharactersInRegex: intentionally stripping control characters from pasted text
			.replace(/[\x00-\x1f]/g, "")
			.trim();
		if (!text) return;
		setValues((prev) => ({
	...
	useKeyboard((key) => {
		...
		if (key.name === "tab" || (key.name === "down" && !key.meta)) {
			setFocusedField(FIELDS[(focusedIndex + 1) % FIELDS.length] as Field);
			return;
		}

		if ((key.shift && key.name === "tab") || key.name === "up") {
			setFocusedField(
				FIELDS[(focusedIndex - 1 + FIELDS.length) % FIELDS.length] as Field,
			);
			return;
		}
		...
		if (key.name === "backspace" && (key.meta || key.option)) {
			setValues((prev) => {
				...
				return {
					...prev,
					[focusedField]: lastSpace >= 0 ? val.slice(0, lastSpace) : "",
				};
			});
			setError(null);
			return;
		...
		>
			...
			>
				...
				<text fg={theme.textDim}>
					<span fg={theme.accent}>Tab</span>/<span fg={theme.accent}>↑↓</span>{" "}
					Navigate{"  "}
